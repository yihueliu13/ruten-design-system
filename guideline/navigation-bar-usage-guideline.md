...
- **layout**：`search`（嵌入 SearchBar）| `title`（顯示頁面標題）| `action`（嵌入 CTA Button Slot）

PC / 桌面版不使用 NavigationBar — PC 的頂部導航是獨立的 DesktopHeader component（含 Logo + 搜尋 + 文字導航），結構完全不同，不共用 NavigationBar 的 token。
...
- **centerSlot**：SearchBar instance（layout=search）/ page title text（layout=title）/ CTA Button instance（layout=action）
...
| **brand** | **action** | 品牌橘 `surface-brand` | 白色 `on-surface-brand` | 橘底頁面帶 CTA（如買直購） |
| **default** | **title** | 白色 `surface` | 深灰 `on-surface` | 競標、商品頁、設定頁等內頁 |
| **default** | **search** | 白色 `surface` | 深灰 `on-surface` | 內頁搜尋模式（展開搜尋框） |
| **default** | **action** | 白色 `surface` | 深灰 `on-surface` | 白底頁面帶 CTA |
...
| 買直購入口 | brand | action | back arrow | 買直購 Button (Slot) | cart + icons |
...
### Don't ✗
- 不要在 PC / 桌面版用 NavigationBar（PC 用獨立的 DesktopHeader component）
- 不要自定義 NavigationBar 的背景色（只有 brand 和 default 兩種）
- 不要在 NavigationBar 裡放 Tab（Tab 是獨立 component，在下方）
- 不要修改嵌入的 SearchBar 的外觀（SearchBar 外觀全局統一，純白背景）
- 不要混淆 NavigationBar 和 SectionHeader（NavigationBar = 頁面導航，SectionHeader = 區塊標題）
- 不要在 layout=action 裡自定義 Button 樣式（Button 用自己的 comp/button token，NavigationBar 只管 Slot 位置）
...
```
1. NavigationBar 僅限 App / Mobile 版使用，PC 用獨立 DesktopHeader
2. 高度固定 48px，不含系統 StatusBar
3. 兩個 variant 維度：
   - surface: brand (品牌色底) | default (白底)
   - layout: search (嵌 SearchBar) | title (頁面標題) | action (嵌 CTA Button)
4. 結構：
   <NavigationBar surface="brand|default" layout="search|title|action">
     <leftSlot>   back arrow | hamburger menu          </leftSlot>
     <centerSlot> SearchBar | page title | CTA Button  </centerSlot>
     <rightSlot>  action icons (24px each)              </rightSlot>
   </NavigationBar>
5. surface=brand → 品牌色底 + 白色文字/icon
   surface=default → 白底 + 深灰文字/icon
6. SearchBar 作為 instance 嵌入，外觀全局統一（純白背景）不隨 surface 變化
7. layout=action 的 CTA Button 是 Slot Override，用 Button component 自己的 token
8. icon 一律 outline style，24px
9. 子元素間距 12px，左右 padding 16px
```
...
├── surface=brand, layout=action    （橘底+CTA Button Slot）
├── surface=default, layout=search  （內頁搜尋，白底+SearchBar）
├── surface=default, layout=title   （內頁標題，白底+標題）
└── surface=default, layout=action  （白底+CTA Button Slot）
```

6 個 variant（2 surface × 3 layout）。共用 height/padding-h/icon-size/gap，差異在 background + icon-color + title-color。
layout=action 的 CTA Button 是 Slot Override — Button component instance 嵌入 centerSlot，樣式由 comp/button token 管理。
...
| v3.0.0 | 2026-03-30 | 新增 layout=action（CTA Button Slot Override）。6 variant（2 surface × 3 layout）。確認桌面版 Header 為獨立 component。SearchBar 背景確認純白。 |
