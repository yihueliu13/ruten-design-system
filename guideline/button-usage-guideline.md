...
- Button 裡的 icon 預設跟文字同色
- Figma 用 comp/button/{role}/default/icon-color token 綁定
- 工程端用 CSS currentColor 繼承，不需要額外處理

```
comp/button/primary/default/icon-color     → {comp.button.primary.default.text-color}
comp/button/secondary/default/icon-color   → {comp.button.secondary.default.text-color}
comp/button/tertiary/default/icon-color    → {comp.button.tertiary.default.text-color}
comp/button/ghost/default/icon-color       → {comp.button.ghost.default.text-color}
```

### icon 位置

- form=label：純文字，無 icon
- form=icon-label：icon 在左（leading），文字在右
- form=label-icon：文字在左，icon 在右（trailing）
- 設計師在 panel 選 form 切換

### Instance Swap

- icon 預設是 outline/home
- 設計師需要手動在 Component 裡設定 Instance Swap Property
- 所有 icon 的 layer name 統一為 "icon"（確保切 variant 時 override 保留）
...
|------|------|------|
| **label** | 純文字 | 預設 |
| **icon-label** | icon 在左 + 文字在右 | ＋關注、✓已完成 |
| **label-icon** | 文字在左 + icon 在右 | 查看賣場→、展開▼ |
