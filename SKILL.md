...
> 682 tokens (ref 160 + sys 170 + comp 352) + 130 Text Styles (130 styles)
...
| 角色 | 負責範圍 |
|------|---------|
| **Cowork（Opus）** | 設計決策、Figma MCP 操作、治理規則衝突判斷、新元件 spec 定義、JSON 寫入、validate、sync、guideline 撰寫、Scripter 腳本產出、研究調查 |
...
| 3 | Token 定義 | Cowork | 完整 token 表格（path, $value, $description） | ← 步驟 7/9/12 發現問題都回這裡 |
...
| 8 | Scripter 腳本 | Cowork | `scripter-XX-{component}.js` | — |
| 9 | Figma Variable 匯入 | Kay | Thierry overlay import（不刪 Collection） | import error → 檢查 JSON 格式 |
| 10 | Figma Component 建置 | Kay | 在 Figma Scripter 執行腳本 | 視覺不對 → 回步驟 3 |
| 11 | Variable Binding | Kay | comp/ token 綁到 Figma Component 屬性 | — |
| 12 | 視覺驗證 | Kay | 截圖比對設計稿 | — |
| 13 | 完整交付驗證 | Cowork | §7a 12 項清單全 PASS | 任一 FAIL → 回步驟 3 |
...
- **Foundation**：ref + sys token → Figma `Foundation › Global + Semantic` / Storybook `Foundations/`
- **Primitive**：最小可複用 UI 單元 → `comp/` 完整集 → Figma `Primitive` / Storybook `Components/{功能類別}/`
...
> **Figma Page 名稱 vs Token 路徑**：Global = `ref/`、Semantic = `sys/`、Primitive + Component = `comp/`。Token 路徑維持業界標準不變。
...
- **文字繼承規則（Typography Variable Binding）** — 見下方 §3a

### §3a 文字繼承規則（Typography Variable Binding）

**核心原則：所有 Figma Component 的 text node 必須綁定 4 項 Variable，不允許 hardcode。**

| # | 屬性 | 綁定目標 | 說明 |
|---|------|---------|------|
| 1 | `fontFamily` | `sys/typography/font-family/primary` | 所有 text node 共用，確保品牌字型一致 |
| 2 | `fontWeight` | `comp/{component}/font-weight` 或 `sys/typography/weight/*` | 按 component × state 區分（如 active/inactive） |
| 3 | `fontSize` | `comp/{component}/{size}/font-size` | 按 size variant 區分 |
| 4 | `fills`（text color） | `comp/{component}/{role}/text-color` 或 Slot Override token | paint-level binding（`paint.boundVariables`，不用 `setBoundVariable("fills")`） |

**綁定現狀（2026-03-26 審計通過）：**

| Component | Text Nodes | fontFamily | fontWeight | fontSize | fills | 狀態 |
|-----------|-----------|------------|------------|----------|-------|------|
| Button | 48 | 48/48 | 48/48 | 48/48 | 48/48 | ✅ |
| Tab Item | 24 | 24/24 | 24/24 | 24/24 | 24/24 | ✅ |
| SectionHeader | 28 | 28/28 | 28/28 | 28/28 | 28/28 | ✅ |
| Avatar | 12 | 12/12 | 12/12 | 12/12 | 12/12 | ✅ |
| Thumbnail | 12 | 12/12 | 12/12 | 12/12 | 12/12 | ✅ |
| SearchBar | 2 | 2/2 | 2/2 | 2/2 | 2/2 | ✅ |
| NavigationBar | 2 | 2/2 | 2/2 | 2/2 | 2/2 | ✅ |
| BottomNav | 4 | 4/4 | 4/4 | 4/4 | 4/4 | ✅ |
| **合計** | **132** | **132** | **132** | **132** | **132** | **✅ 0 unbound** |

**新增 component 時的檢查清單：**

1. JSON 定義 comp token：`font-size`、`font-weight`、`text-color`（每個 text node 角色都要）
2. Figma 建立對應 Variable（alias 到 sys 層）
3. Figma 綁定 4 項屬性到每個 text node
4. MCP 雲端無 PingFang TC → 用 Noto Sans TC 替代 → fontFamily 綁定 `sys/typography/font-family/primary` 後，在用戶機器自動顯示 PingFang TC
5. **綁定完成後必須跑審計腳本確認 0 unbound**

**允許的例外（必須在 $description 註明原因）：**

| 例外類型 | 說明 | 範例 |
|---------|------|------|
| comp 層 hardcode < base-4 | 值小於 4px，不在 spacing scale 上 | `tab/indicator-height = 3`, `tag/icon-gap = 2` |
| 負值位移 | 無 sys 等價，純定位用 | `badge/offset-x = -4` |
| CSS 字串值 | 非數值型 token | `aspect-ratio = 1/1` |
| 計算衍生值 | 由 layout 公式得出，不在 base-4 scale | `product-card/width = 109`（`(375-2×16-2×8)/3`） |
| line-clamp | CSS 行數控制，非尺寸 token | `title/line-clamp = 2` |

**comp 層 hardcode 現狀：327 tokens 中 9 個 hardcode，全部有 $description 說明原因。**
...
| 2 | Button | Action | Primitive | 60 | 步驟 11 視覺驗證 | ✅ Figma Component 48 variants (4 role × 4 size × 3 form) |
| 3 | Tab | Navigation | Primitive | 33 | 步驟 11 視覺驗證 | ✅ Figma Component 24 variants (2 emphasis × 4 size × 3 state) |
| 4-6 | Tag (Filter/Display/Promo) | Feedback | Primitive | 74 | 步驟 12 交付驗證 | ✅ 清理完成，無舊路徑殘留 |
| 7 | Icon | Media | Primitive | 8 | 步驟 12 交付驗證 | ✅ 476 icons, vector fill bound to Variable |
| 8 | Badge | Feedback | Primitive | 11 | 步驟 4 JSON 完成 | ✅ Token 完成，待建 Figma Component |
| 9 | SectionHeader | Layout | Compound | 14 | 步驟 4 JSON 完成 | ✅ Token 完成 |
...
| 19 | Avatar | Media | Primitive | 11 | 步驟 11 視覺驗證 | ✅ Figma Component 12 variants |
| 20 | Thumbnail | Media | Primitive | 11 | 步驟 11 視覺驗證 | ✅ Figma Component 12 variants |
...
Cowork（Opus）直接掛載 repo 讀取所有檔案，不再需要維護 Project Knowledge 快照。

- **每次開新 session**：Cowork 自動讀 `CLAUDE.md` + `SKILL.md`（由 skill trigger）
- **跨 session 記憶**：auto-memory 持久化（`/mnt/.auto-memory/`）
- **最新數據**：直接從 repo 讀 `design-system-all.json`、`component-governance.md` 等

> **已棄用**：Chat Project Knowledge 手動上傳機制。所有檔案以 repo 為唯一真實來源。
