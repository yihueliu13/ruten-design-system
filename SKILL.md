...
> 686 tokens (ref 160 + sys 170 + comp 356) + 130 Text Styles (130 styles)
...
詳細規則、決策流程、Registry 見 `ref/component-governance.md`。
...
- Thierry Community 版只能 update 既有 Variables，不能 create → 新 Variable 一律用 Scripter 建立
- design-system-all.json 色彩格式用 hex string（如 `"#FF963B"`），不用 Figma REST API object 格式
...
| Scripter（Figma Plugin） | 在 Figma 內跑 JS 腳本建 Component / 批次操作 / **建立新 Variable**（Thierry 不能 create） |
| Thierry（Figma Plugin） | Variable overlay import — **只能 update 既有 Variables**，不能 create 新的。新 Variable 一律走 Scripter。 |
| `tool/validate-design-system.py` | 每次改 JSON 後必跑，檢查 alias 完整性 + 衍生檔數字一致 |
| `tool/sync-derived-files.py` | 自動更新所有衍生檔中的 token 數字 |
| `script/create-text-styles.js` | Scripter 腳本，產生 130 Text Styles（CH 65 + EN 65） |
...
| **ref/** | **架構參考 & 治理** |
| `ref/component-governance.md` | 三維分類體系 + Primitive/Compound/Pattern Registry + controlHeight + Slot Override + 鎖定決策 + 架構決策記錄 |
| `ref/icon-inventory.json` | 476 icons 清單 |
| **guideline/** | **元件使用規範** |
| `guideline/button-usage-guideline.md` | Button 使用規則（4 role × 5 size × state × form × iconPosition × Wrapper） |
| `guideline/tab-usage-guideline.md` | Tab 使用規則（4 size × 3 state × 2 layout × 2 emphasis + 高度公式） |
| `guideline/avatar-usage-guideline.md` | Avatar 使用規則（人/商家 only） |
| `guideline/thumbnail-usage-guideline.md` | Thumbnail 使用規則（IP/主題/分類，Shopify Polaris 模式） |
| `guideline/tag-badge-usage-guideline.md` | Tag/Badge 使用規則 |
| `guideline/banner-usage-guideline.md` | Banner 使用規則 |
| `guideline/divider-usage-guideline.md` | Divider 使用規則 |
| `guideline/section-header-usage-guideline.md` | SectionHeader 使用規則 |
| `guideline/section-module-usage-guideline.md` | SectionModule 使用規則 |
| `guideline/navigation-bar-usage-guideline.md` | NavigationBar 使用規則 |
| `guideline/search-bar-usage-guideline.md` | SearchBar 使用規則 |
| `guideline/bottom-nav-usage-guideline.md` | BottomNav 使用規則 |
| **script/** | **Figma Scripter 腳本** |
| `script/create-text-styles.js` | Figma Scripter 腳本（130 Text Styles） |
| `script/scripter-04-sprint3-variables.js` | Sprint 3 Variable 建立 |
| `script/scripter-05-navigation-bar.js` | NavigationBar Component |
| `script/scripter-06-navbar-binding.js` | NavigationBar Variable Binding |
| `script/scripter-07-search-bar.js` | SearchBar Component |
| `script/scripter-08-bottom-nav.js` | BottomNav Component |
| **tool/** | **驗證 & 同步工具** |
| `tool/validate-design-system.py` | 驗證腳本（alias、數字一致、legacy 殘留） |
| `tool/sync-derived-files.py` | 衍生檔 token 數字自動同步 |
| `tool/sync-daily-log.py` | DAILY_LOG 全生命週期管理 |
| **viewer/** | **視覺化預覽** |
| `viewer/ruten-design-system.html` | 即時 viewer（fetch JSON，瀏覽器開） |
| `viewer/design-system-hierarchy.html` | Token 階層結構圖 |
| `viewer/ruten-ds-architecture.jsx` | 檔案關係圖（React） |
| **ops/** | **營運 & 進度追蹤** |
| `ops/EXECUTION_PLAN.md` | 19 元件清單 + Sprint 順序 + Phase 2-4 路線圖 + 技術債 |
| `ops/DAILY_LOG.md` | 每日站會記錄（每天第一件事：讀最新一天的「明日工作順序」） |
...
進度數字會變動，不寫死在 SKILL.md。查進度一律讀 ops/ 即時檔案：

- **元件進度 + Sprint 狀態** → `ops/EXECUTION_PLAN.md`
- **每日工作記錄** → `ops/DAILY_LOG.md`
- **Token 即時數字** → `python3 tool/validate-design-system.py --root .`
