...
| `ops/EXECUTION_PLAN.md` | 19 元件清單、優先級、執行順序、Phase 2-4 路線圖、技術債 | Claude, Kay |
| `ops/DAILY_LOG.md` | 每日站會記錄 | Claude, Kay |
| `tool/validate-design-system.py` | 驗證腳本 | Claude（每次改 JSON 後跑） |
| `tool/sync-derived-files.py` | 自動更新衍生檔 token 數字 | Claude（每次改 JSON 後跑） |
| `tool/sync-daily-log.py` | DAILY_LOG 生命週期管理 | Claude（啟動/收工） |
| `ref/component-governance.md` | Component 分類 + token 規則 + RWD 治理 + 鎖定決策 + 架構決策記錄 | Claude, 設計師, 工程師 |
| `guideline/*.md` | 各元件使用規範（12 份） | Claude, 設計師 |
| `script/create-text-styles.js` | Figma Scripter 腳本（130 Text Styles） | Figma plugin |
| `viewer/ruten-design-system.html` | Design System viewer（內嵌 JSON） | Kay（瀏覽器開） |
...
- 目前：686 tokens + 130 Text Styles

### 元件分類（v2 三維體系）

詳細規則見 `ref/component-governance.md`。以下為摘要：
...
4. DO       執行任務
5. CHECK    python3 tool/validate-design-system.py --root .
6. SYNC     python3 tool/sync-derived-files.py --root .（自動更新衍生檔數字）
...
| 6 | 衍生檔數字同步 | tool/sync-derived-files.py | ref/component-governance / ops/EXECUTION_PLAN / CLAUDE 數字一致 |
...
- 刪除檔案前，必須先跑 `grep -rl "檔名" .` 確認無其他檔案引用，再跑 `python3 tool/validate-design-system.py --root .` 確認通過。兩步都過才能刪
...
5. 驗證 JSON     python3 tool/validate-design-system.py --root .
6. 同步衍生檔    python3 tool/sync-derived-files.py --root .
7. Usage Guideline  為每個新 component 寫 guideline/{component}-usage-guideline.md
8. Scripter 腳本  為每個新 component 寫 script/ 裡的 Figma Scripter 腳本
